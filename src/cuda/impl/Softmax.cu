...
namespace mlp {
    __global__ void softmax_kernel(const float* logits, float* activations, const size_t rows, const size_t cols) {
        ...
        */
        ...
        for (unsigned int col = tid; col < cols; col += stride) {
            partial_sum += expf(logits[row * cols + col] - max_value);
        }
        ...
        */
        ...
        for (unsigned int col = tid; col < cols; col += stride) {
            activations[row * cols + col] = expf(logits[row * cols + col] - max_value) / sum;
        }
    }
    ...
    void Context::softmax(const Matrix& logits, Matrix& activations) const {
        ...
        assert(logits.columns() < BLOCK_SIZE);

        cudaError_t err;

        if (m_profiler) m_profiler->startTask("Softmax");

        softmax_kernel<<<logits.rows(), BLOCK_SIZE>>>(logits.data(), activations.data(), logits.rows(), logits.columns());
