...
namespace mlp {
    ...
    template<typename TActivation = NoActivation>
    __global__ void propagate_kernel_old(
        const float* input, float* output, const float* weights, const float* biases,
        const size_t batch_size, const size_t input_count, const size_t output_count
    ) {
        unsigned int row = blockDim.y * blockIdx.y + threadIdx.y;
    ...
    void DeviceContext::propagateOld(const Matrix& input, Matrix& output, const Matrix& weights, const Matrix& biases, const Activation activation) const {
        assert(input.columns() == weights.rows());
        assert(output.rows() == input.rows());
        assert(output.columns() == weights.columns());
        ...
        switch (activation) {
            case Activation::SIGMOID:
                propagate_kernel_old<Sigmoid><<<grid, block>>>(input.data(), output.data(), weights.data(), biases.data(), input.rows(), input.columns(), output.columns());
            break;

            case Activation::TANH:
                propagate_kernel_old<Tanh><<<grid, block>>>(input.data(), output.data(), weights.data(), biases.data(), input.rows(), input.columns(), output.columns());
            break;

            case Activation::RELU:
                propagate_kernel_old<ReLU><<<grid, block>>>(input.data(), output.data(), weights.data(), biases.data(), input.rows(), input.columns(), output.columns());
            break;

            case Activation::LEAKY_RELU:
                propagate_kernel_old<LeakyReLU><<<grid, block>>>(input.data(), output.data(), weights.data(), biases.data(), input.rows(), input.columns(), output.columns());
            break;

            case Activation::SOFTMAX:
                propagate_kernel_old<<<grid, block>>>(input.data(), output.data(), weights.data(), biases.data(), input.rows(), input.columns(), output.columns());
